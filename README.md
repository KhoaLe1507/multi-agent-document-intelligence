...
Dự án được khuyến nghị dùng Python >=3.10. Bạn có thể sử dụng Venv, UV, PDM, hay Poetry tùy ý (có sẵn file `uv.lock`, `pyproject.toml`).
Bắt buộc cần cấu hình API Keys:
```bash
cp .env.example .env
```
Mở tệp `.env` và nhập `OPENAI_API_KEY` cũng như cấu hình máy chủ liên quan.
